...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if (!/^\/https?:/.test(req.url) && !isValidHostName(location.hostname)) {
      // If this doesn't have a host we add one
      req.url = '/0.0.0.0:8081' + req.url
      location = parseURL(req.url.slice(1));
      
      // COMMENTING THIS OUT: The main polymer app generates requests for '/src/other-components'
      //   and we want to redirect those to the polymer server (which is on port 8081)
      // -----------------------------------------------------------------------------------------
