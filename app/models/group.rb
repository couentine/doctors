class Group
  ...
  belongs_to :creator, inverse_of: :created_groups, class_name: "User", dependent: :nullify
  ...
  validates :type, inclusion: { in: VALID_TYPE_VALUES, message: "%{value} is not a valid Group Type" }
  validates :customer_code, presence: { message: "is required for private Groups" }, if: :private?
  validates :customer_code, inclusion: { in: VALID_CUSTOMER_CODE_VALUES, allow_nil: true, message: "%{value} is not a valid customer code" }, if: :private?
