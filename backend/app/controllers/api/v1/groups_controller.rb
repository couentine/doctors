class Api::V1::GroupsController < Api::V1::BaseController
  ...
  # This can be accessed via the groups index and the user groups index.
  def index
    skip_authorization
    ...
    group_criteria = group_criteria.order_by(@sort_string).page(@page[:number]).per(@page[:size])
    set_calculated_pagination_variables(group_criteria)
    @groups = group_criteria.entries

    @policy = GroupPolicy.new(@current_user, @groups)
